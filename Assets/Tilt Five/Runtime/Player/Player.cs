...
namespace TiltFive
{
    ...
    public class Player : Singleton<Player>
    {
        ...
        internal static void OnEnable(uint supportedPlayers)
        {
#if UNITY_2019_1_OR_NEWER && INPUTSYSTEM_AVAILABLE
            var glassesDevices = Input.glassesDevices;
            var wandDevices = Input.wandDevices;
            var leftIndex = (int)ControllerIndex.Left;
            var rightIndex = (int)ControllerIndex.Right;

            for (var playerIndex = PlayerIndex.One; playerIndex <= PlayerIndex.Four; playerIndex++)
            {
                var i = (int)playerIndex - 1;
                if ((uint)playerIndex > supportedPlayers)
                {
                    // Remove a GlassesDevice if possible
                    if (glassesDevices[i] != null)
                    {
                        InputSystem.RemoveDevice(glassesDevices[i]);
                        glassesDevices[i] = null;
                    }

                    // Remove two WandDevices if possible
                    if (wandDevices[i, leftIndex] != null)
                    {
                        InputSystem.RemoveDevice(wandDevices[i, leftIndex]);
                        wandDevices[i, leftIndex] = null;
                    }
                    if (wandDevices[i, rightIndex] != null)
                    {
                        InputSystem.RemoveDevice(wandDevices[i, rightIndex]);
                        wandDevices[i, rightIndex] = null;
                    }
                }
                else
                {
                    ...
                    // Create two WandDevices if necessary
                    Input.AddWandDevice(playerIndex, ControllerIndex.Left);
                    Input.AddWandDevice(playerIndex, ControllerIndex.Right);
                    InputSystem.QueueConfigChangeEvent(wandDevices[i, leftIndex]);
                    InputSystem.QueueConfigChangeEvent(wandDevices[i, rightIndex]);

                    // Ensure that the wands are associated with their parent glasses
                    glassesDevices[i].LeftWand = wandDevices[i, leftIndex];
                    glassesDevices[i].RightWand = wandDevices[i, rightIndex];
                }

                if(Application.isPlaying)
                {
                    // Disable the T5 devices at the start of runtime.
                    // As glasses/wands connect, they will enable their respective devices.
                    if (glassesDevices[i] != null)
                    {
                        InputSystem.DisableDevice(glassesDevices[i]);
                    }
                    if (wandDevices[i, leftIndex] != null)
                    {
                        InputSystem.DisableDevice(wandDevices[i, leftIndex]);
                    }
                    if (wandDevices[i, rightIndex] != null)
                    {
                        InputSystem.DisableDevice(wandDevices[i, rightIndex]);
                    }
                }
            }
#endif
